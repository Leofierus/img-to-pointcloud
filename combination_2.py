...
for i in tqdm(range(0, len(pcds)-1)):
    draw_pcds(pcds[i], pcds[i+1])

    pcd0_down, pdc0_fpfh = preprocess_point_cloud_2(pcds[i], voxel_size)
    pcd1_down, pdc1_fpfh = preprocess_point_cloud_2(pcds[i+1], voxel_size)

    result = execute_global_registration(pcd0_down, pcd1_down, pdc0_fpfh, pdc1_fpfh, voxel_size)


    evaluation = o3d.pipelines.registration.evaluate_registration(
            pcds[i], pcds[i+1], threshold, result.transformation)
    print(evaluation)

    #pcds[0].estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=100, max_nn=30))
    #pcds[1].estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=100, max_nn=30))
    ...
    draw_registration_result(pcds[i], pcds[i+1], registered_images)

    #o3d.visualization.draw_geometries([pcds[0], pcds[1]])
    temp_pointclouds = [pcds[i], pcds[i+1]]

    with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
        pose_graph = full_registration(temp_pointclouds, result)

    pcd_combined = o3d.geometry.PointCloud()
    pcd_combined = pcds[i].transform(pose_graph.nodes[0].pose)
    pcd_combined += pcds[i+1].transform(pose_graph.nodes[1].pose)
    pcd_combined_down = pcd_combined.voxel_down_sample(voxel_size=0.05)
    all_pcds.append(pcd_combined_down)
