...
def optimize_distortion_params(observed_points1, observed_points2, K, dist_initial):
    ...
    result = least_squares(reprojection_error, [dist_initial[0], dist_initial[1]],
                           args=(normalized_points1, observed_points2, K))
    return result.x
...
for i in tqdm(range(len(image_paths))):
    ...
    dist_initial = [dist_coeffs[0] * 500, dist_coeffs[1] * 50, 0, 0, 0]
    ds.append(dist_initial)
    ...
    if i % 15 == 0 or i == len(image_paths) - 1:
        ...
        o3d.visualization.draw_plotly([temp_pcd], mesh_show_wireframe=True)
        exit()
